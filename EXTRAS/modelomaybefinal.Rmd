...
Se afirma que existe una correlación entre la temperatura y la cantidad de accidentes porque el p-value es menor que 0.05
...
En esta información se puede apreciar que el p valor es muy pequeño, lo que indica que hay al menos una variable que está relacionada con la variable que se busca predecir. Además el Rˆ2 = 68.86%

Para ver las variables que están mas relacionadas 
```{r}
summary(modelo)$coefficient
```

Se puede observar que la variable viento y precipitación tienen un p-valor > 0.05 por lo que se eliminan del modelo


```{r}
modelo1<-lm(cant_accidentes ~ id_desc_dia_semana + temperatura + 
    horario + estacion, data = datos3)
```
...
El p-valor, luego de eliminar la variable viento y precipitación sigue siendo muy pequeño por lo que el modelo es correcto.



Ahora aplicamos el modelo para predecir la cantidad de accidentes que pueden ocurrir si es de noche (horario=1), un viernes (id_desc_dia_Semana =5), en otoño (estacion=3), un día cálido (temperatura=2) 

```{r}
predict(modelo1, data.frame(horario=1, id_desc_dia_semana=5, temperatura=2, estacion=3))
```

Pueden ocurrir una cantidad de 27.72 accidentes.
