...
```{r}
datos5<- datos4[,c("id_desc_tipo_victima", "id_desc_condicion_fisica", "id_desc_sexo", "id_desc_responsabilidad_peaton", "edad")]
head(datos5)
```
...
```{r}
datos6 <- datos5 %>%
  group_by(edad,id_desc_tipo_victima , id_desc_condicion_fisica, id_desc_sexo, id_desc_responsabilidad_peaton) %>%
...
```{r}
model.rf <- randomForest(formula = cantvictimas ~ ., importance = TRUE ,data = entrevali$train)
varImpPlot(model.rf)
