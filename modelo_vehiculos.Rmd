...
Importaci贸n data de veh韈ulos
```{r}
vehiculo <- dbGetQuery(con, "SELECT * FROM vehiculo_involucrado")
head(vehiculo)
```

Importaci贸n tablas relacionadas con veh韈ulos 
```{r}
...
Resumen de veh韈ulos seg鷑 su tipolog韆
...
kable( datos_grafico , caption = "BBDD `airquality` con `kable()`"
       , align = c('l', 'c', 'r', 'r', 'c', 'l')
       , col.names = c("Tipo de veh韈ulo", "Cantidad 2015 a 2017")
       , row.names = TRUE
       , digits = 1
      )
